...
class DrawSession
{
  public:
    ...
    void DrawTextCentered(const std::string &text, int x, int y, int fontSize, Color color) noexcept;
    void DrawFPS(int posX, int posY, int height);
    void DrawTexture(const Texture2D &texture, int x, int y, Color color) noexcept;
    void DrawTexture(const Texture2D &texture, float x, float y, float width, float height, const Vector2 &pos, Color color) noexcept;
    void DrawTexture(const Texture2D &texture, const Rectangle &source, const Vector2 &pos, Color color) noexcept;
    // Wrapper for raylib::DrawTexturePro without rotation; rotation forced to 0
    void DrawTexturePro(const Texture2D &texture, const Rectangle &source, const Rectangle &dest, const Vector2 &origin, float scale,
                        Color color) noexcept;
    // Wrapper for raylib::DrawTextureEx to allow scaling when drawing textures (no rotation)
    void DrawTextureEx(const Texture2D &texture, const Vector2 &pos, float scale, Color color) noexcept;
    // Convenience overload using scalar position instead of Vector2 (no rotation)
    void DrawTextureEx(const Texture2D &texture, float x, float y, float scale, Color color) noexcept;
